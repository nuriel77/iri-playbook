...
def main():
    ...
    for i, service in enumerate(services):
        ...
        if scheme == 'http':
            data_output.append("%sserver %s %s:%s weight %s maxconn %s tfo alpn h2,http/1.1 resolvers dns resolve-prefer ipv4 check" %
                               (indent, service['ID'], service['Address'], service['Port'], weight, maxconn))
        elif scheme == 'https':
            data_output.append("%sserver %s %s:%s weight %s maxconn %s tfo alpn h2,http/1.1 resolvers dns resolve-prefer ipv4 check ssl verify %s %s" %
