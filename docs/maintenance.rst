...
In the following example we assume that the new version is **1.4.2.2**.
...
.. code:: bash
   ...
   export IRIVER=1.4.2.2 ; curl -L "https://github.com/iotaledger/iri/releases/download/v${IRIVER}/iri-${IRIVER}.jar" --output "/var/lib/iri/target/iri-${IRIVER}.jar"
...
In **Ubuntu**::
  ...
  sed -i 's/^IRI_VERSION=.*$/IRI_VERSION=1.4.2.2/' /etc/default/iri

In **CentOS**::

  sed -i 's/^IRI_VERSION=.*$/IRI_VERSION=1.4.2.2/' /etc/sysconfig/iri
...
.. code:: bash
  ...
  ps aux|grep iri-1.4.2.2|grep -vq grep && echo found
